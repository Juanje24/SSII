...
float SBR_FC::calcular_fc(string meta, int profundidad, ofstream &ostr)
{
    ...
    if (n > 0)
    {
        ...
        for (int i = 0; i < m - 1; i++)
        {
            ...
            if (FCs.at(i) >= 0 && FCs.at(i + 1) >= 0)
                FCs.at(i + 1) = FCs.at(i) + FCs.at(i + 1) * (1 - FCs.at(i)); // Caso 2.1
            else if (FCs.at(i) <= 0 && FCs.at(i + 1) <= 0)
                FCs.at(i + 1) = FCs.at(i) + FCs.at(i + 1) * (1 + FCs.at(i)); // Caso 2.2
            else
                FCs.at(i + 1) = (FCs.at(i) + FCs.at(i + 1)) / (1 - min(abs(FCs.at(i)), abs(FCs.at(i + 1)))); // Caso 2.3
        }

        // Añadimos la meta como un hecho, con el FC calculado HECHO
...
void SBR_FC::encadenamiento_hacia_atras(ofstream &ostr)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (fc >= -1 && fc <= 1)
            ostr << "La meta " << SBR_FC::metas.at(i) << " es cierta con FC = " << fc << endl;
        else
            cout << "Se ha producido algún error" << endl;
    }
}
